...
class Help:
    def __init__(self, partner):
        ...
        help_text = 'This is a multi choice quiz, use your knowledge of rock and roll bands to pick which band the member is from. use the stats button to check your statistics. Good Luck :)'
...
class DisplayStats:
    def __init__(self, partner):
        ...
        self.stats_title = Label(self.stats_frame, text = 'Statistics', fg = '#000000', bg = '#B5739D', wraplength=250, font = ('Arial', '16', 'bold'))
        self.stats_title.grid(row=0, columnspan=2,padx=10,pady=10)

        self.stats_label = Label(self.stats_frame, text = 'Your total accuracy = ', fg = '#000000', bg = '#E1D5E7', wraplength=250,font=('Arial','12'))
        self.stats_label.grid(row=1,column=0,padx=10,pady=10)


        # this is to see if the user has won more then 1 round, if they havent there accuracy is zero %
        if partner.rounds_won.get() > 0:
            accuracy = (partner.rounds_won.get() / partner.rounds_played.get()) * 100
        else:
            accuracy = 0
        ...
        self.stats_dismiss.grid(row=2,columnspan=2,padx=5,pady=5)
